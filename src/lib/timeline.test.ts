...
export function runProductionTimelineTests() {
  ...
  // TEST 1: Create project. Expected: Files Received = ACTIVE, 2 production days.
  ...
  console.assert(summary.timelineStatus === 'Active', 'Test 1 Failed: Status should be Active');
  console.assert(
    getStageDurationDays('Files Received', project.workflow_settings) === 2,
    'Test 1 Failed: Allocation should be 2 days',
  );
  console.log('✓ TEST 1 PASSED: Files Received = ACTIVE (2 production days)');

  // TEST 2: Complete Files Received. Expected: Design Concept = ACTIVE, 3 production days allocated.
  ...
  console.log('✓ TEST 2 PASSED: Design Concept = ACTIVE (3 production days)');

  // TEST 3: Complete Design Concept and send to client. Expected: Concept Approval = PAUSED_CLIENT_REVIEW, Waiting On = Client.
  ...
  console.log('✓ TEST 3 PASSED: Concept Approval = PAUSED (Waiting for Client)');
  ...
  // TEST 5: Client requests revision. Expected: REVISION_ACTIVE, 2 production days allocated, Waiting On = Manuscript Heaven.
  ...
  console.log('✓ TEST 5 PASSED: Concept Revision = REVISION_ACTIVE (2 production days)');

  // TEST 6: Employee completes revision and sends to client. Expected: Clock pauses, Waiting On = Client.
  project.stage_status = 'PAUSED_CLIENT_REVIEW';
  project = deriveProjectTimeline(project);
  summary = getTimelineSummary(project);
  ...
  console.log('✓ TEST 6 PASSED: Revision submitted, clock paused');

  // TEST 7: Client approves. Expected: Concept Approval completed, Print Version becomes ACTIVE (5 production days).
  ...
  console.log('✓ TEST 7 PASSED: Print Version = ACTIVE (5 production days)');

  // TEST 8: Print Version completed. Expected: Print Approval becomes active, Clock pauses, Waiting On = Client.
  ...
  console.log('✓ TEST 8 PASSED: Print Approval = PAUSED (Waiting for Client)');

  // TEST 9: Client requests Print revision. Expected: 2 production days allocated, Clock resumes.
  project.stage_status = 'REVISION_ACTIVE';
  ...
  console.log('✓ TEST 9 PASSED: Print Revision = REVISION_ACTIVE (2 production days)');

  // TEST 10: Client approves Print Version. Expected: Ebook Version becomes ACTIVE (5 production days).
  ...
  console.log('✓ TEST 10 PASSED: Ebook Version = ACTIVE (5 production days)');

  // TEST 11: Ebook Version completed. Expected: Ebook Approval becomes active, Clock pauses.
  ...
  console.log('✓ TEST 11 PASSED: Ebook Approval = PAUSED (Waiting for Client)');

  // TEST 12: Client requests Ebook revision. Expected: 2 production days allocated.
  project.stage_status = 'REVISION_ACTIVE';
  ...
  console.log('✓ TEST 12 PASSED: Ebook Revision = REVISION_ACTIVE (2 production days)');

  // TEST 13: Client approves Ebook. Expected: Final Delivery becomes ACTIVE (2 production days).
  ...
  console.log('✓ TEST 13 PASSED: Final Delivery = ACTIVE (2 production days)');
  ...
  console.log('✓ TEST 14 PASSED: Project & All 8 Stages Completed!');
