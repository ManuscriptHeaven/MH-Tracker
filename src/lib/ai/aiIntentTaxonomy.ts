...
export const INTENT_TAXONOMY: IntentDefinition[] = [
  // --- TASKS ---
  {
    ...
    keywords: {
      ...
      romanUrdu: ['task banao', 'task add karo', 'nayi task', 'task create kro', 'banao', 'task bana do'],
      urdu: ['ٹاسک بناؤ', 'نیا ٹاسک', 'ٹاسک شامل کریں'],
    },
    patterns: [
      /\b(create|add|new|make)\s+.*?\s*task\b/i,
      /\btask\s+(banao|bana\s+do|bna\s+do|add|create|kro|kar\s+do|bnao)\b/i,
      /\b(nayi|naya)\s+task\b/i,
      /ٹاسک\s+(بناؤ|شامل|کریٹ)/u,
      /نیا\s+ٹاسک|نیا\s+task/iu,
      /کی\s+task\s+بناؤ/iu,
    ],
  ...
  },
  {
    ...
    keywords: {
      english: ['assign task', 'give task', 'assign to', 'put on list', 'assign', 'reassign'],
  ...
  // --- REMINDERS & NOTES & CALENDAR ACTIONS ---
  {
    name: 'update_task',
    category: 'update',
    description: 'Update task properties such as title, deadline, priority, or status',
    keywords: {
      english: ['update task', 'change deadline', 'change title', 'move task', 'rename task'],
      romanUrdu: ['deadline change karo', 'deadline kar do', 'rename kar do', 'move kar do', 'kar do'],
      urdu: ['ڈیڈ لائن تبدیل کریں', 'نام تبدیل کریں'],
    },
    patterns: [
      /\b(change|update|move|set|extend|rename)\s+.*?\s*(deadline|due\s+date|title|task|priority|to)\b/i,
      /\b(deadline|date|title|priority)\s+.*?\s*(change|kar\s+do|krdo|kardo|kar\s+den)\b/i,
      /\b(is|ye|isay)\s+task\s+ki\s+(deadline|date)\b/i,
      /\bmake\s+it\s+due\b/i,
      /کی\s+deadline\s+.*?کر\s+دو/iu,
      /اس\s+task\s+کی/iu,
    ],
  ...
  },
  {
    ...
    keywords: {
      english: ['create project', 'new project', 'add project', 'make project'],
      romanUrdu: ['project banao', 'project add karo', 'naya project', 'project create karo'],
      urdu: ['نیا پروجیکٹ', 'پروجیکٹ بناؤ'],
    },
    patterns: [
      /\b(create|add|new|make)\s+.*?\s*project\b/i,
      /\bproject\s+(banao|add|create|kro|kar do|bnao)\b/i,
      /\bnaya\s+project\b/i,
      /نیا\s+پروجیکٹ|پروجیکٹ\s+بناؤ/u,
    ],
    recommendedTool: 'create_project',
  },
  {
    name: 'update_project',
    category: 'update',
    description: 'Update project deadline, title, client, or status',
    keywords: {
      english: ['update project', 'change project deadline', 'extend project', 'rename project'],
      romanUrdu: ['project ki deadline', 'project extend karo', 'project deadline kar do'],
  ...
  },
  {
    ...
    keywords: {
      english: ['create reminder', 'remind me', 'set reminder', 'add reminder'],
      romanUrdu: ['reminder banao', 'mujhe yaad dilana', 'remind me', 'reminder add karo'],
      urdu: ['یاد دہانی', 'مجھے یاد دلانا'],
    },
    patterns: [
      /\bremind\s+me\b/i,
      /\b(create|set|add)\s+.*?\s*reminder\b/i,
      /\b(reminder|yad)\s+(banao|add|karo|dilana)\b/i,
      /\bkal\s+.*?\s*baje\s+mujhe\b/i,
      /مجھے\s+یاد\s+دلانا|یاد\s+دہانی/u,
    ],
    recommendedTool: 'create_reminder',
  },
  {
    name: 'update_calendar',
    category: 'update',
    description: 'Schedule or update calendar event or meeting',
    keywords: {
      english: ['put meeting', 'schedule meeting', 'move meeting', 'calendar event', 'add meeting'],
      romanUrdu: ['meeting rakho', 'meeting schedule karo', 'calendar pe dalo', 'meeting move karo'],
      urdu: ['میٹنگ رکھیں', 'کیلنڈر اپ ڈیٹ'],
    },
    patterns: [
      /\b(put|schedule|add|move|change)\s+.*?\s*(meeting|event|appointment|calendar)\b/i,
      /\bmeeting\s+.*?\s*(rakho|schedule|kar\s+do|baje)\b/i,
      /\bcalendar\s+pe\s+(dalo|rakho|add)\b/i,
      /میٹنگ\s+(رکھیں|شیڈول)/u,
    ],
    recommendedTool: 'update_calendar',
  },
  {
    name: 'add_note',
    category: 'create',
